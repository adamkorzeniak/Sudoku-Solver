...
public class XYWingSquare {
	...
	public static List<Field> getEligibleFields(Board board, Field field, List<Field> horizontal,
			List<Field> vertical) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				...
				if (toCheck.getPossibleCount() == 2 && !toCheck.equals(field) && !horizontal.contains(toCheck)
						&& !vertical.contains(toCheck)) {
					result.add(toCheck);
				}
			}
		}
